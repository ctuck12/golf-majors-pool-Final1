...
export async function GET(request: Request) {
  ...
  const cacheKey = isTournament
    ? `player-stats:v32:tourn:${eventId}:${name}`
    : `player-stats:v48:season:2026:${name}`;
  const ranksCacheKey = isTournament
    ? `player-stats:v32:tourn:${eventId}:${name}${RANKS_CACHE_SUFFIX}`
    : `player-stats:v48:season:2026:${name}${RANKS_CACHE_SUFFIX}`;
