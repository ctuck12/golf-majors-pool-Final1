...
export async function GET(request: Request) {
  ...
  const cacheKey = isTournament
    ? `player-stats:v31:tourn:${eventId}:${name}`
    : `player-stats:v47:season:2026:${name}`;
  const ranksCacheKey = isTournament
    ? `player-stats:v31:tourn:${eventId}:${name}${RANKS_CACHE_SUFFIX}`
    : `player-stats:v47:season:2026:${name}${RANKS_CACHE_SUFFIX}`;
