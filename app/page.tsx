                                ...
                                return (
                                  <Fragment key={player.playerId}>
                                    ...
                                    >
                                      ...
                                      <td style={{ padding: isMobile ? '6px 4px' : '7px 8px', textAlign: 'center', fontWeight: colIsCut ? 600 : 700, color: colUnderPar ? '#dc2626' : (colVal === 'E' ? '#16a34a' : (colIsCut ? '#374151' : '#0f1720')) }}>{player.score === 'CUT' && player.originalScore && leaderboardSortMode === 'default' ? <span onClick={(e) => handleCutClick(String(player.playerId), e)} style={{ cursor: 'pointer', display: 'inline-block', minWidth: 32, textAlign: 'center' }}><span key={expandedCutIds.has(String(player.playerId)) ? 'exp' : 'cut'} className="cut-score-fade">{expandedCutIds.has(String(player.playerId)) ? player.originalScore : 'CUT'}</span></span> : colVal}</td>
                                ...
                                return (
                                  <Fragment key={player.id}>
                                    ...
                                    >
                                      <td style={{ padding: isMobile ? '6px 4px' : '7px 8px', textAlign: 'center', fontWeight: 600, color: '#374151' }}>{formatLeaderboardPosition(player.position)}</td>
                                      <td style={{ padding: isMobile ? '6px 4px' : '7px 8px', fontWeight: activePlayer ? 800 : 500, color: '#0f1720' }}>{player.name}</td>
                                      <td style={{ padding: isMobile ? '6px 4px' : '7px 8px', textAlign: 'center', fontWeight: colIsCut ? 600 : 700, color: colUnderPar ? '#dc2626' : (colVal === 'E' ? '#16a34a' : (colIsCut ? '#374151' : '#0f1720')) }}>{player.score === 'CUT' && player.originalScore && leaderboardSortMode === 'default' ? <span onClick={(e) => handleCutClick(String(player.id), e)} style={{ cursor: 'pointer', display: 'inline-block', minWidth: 32, textAlign: 'center' }}><span key={expandedCutIds.has(String(player.id)) ? 'exp' : 'cut'} className="cut-score-fade">{expandedCutIds.has(String(player.id)) ? player.originalScore : 'CUT'}</span></span> : colVal}</td>
                                      <td style={{ padding: isMobile ? '6px 4px' : '7px 8px', textAlign: 'center', color: '#374151' }}>{(() => {
                                        const isLive = selectedTournamentStatus?.label === 'IN PROGRESS';
                                        const isCutStatus = player.score === 'CUT' || player.score === 'MDF' || player.score === 'WD' || player.score === 'DQ';
                                        if (isLive && !isCutStatus && player.thru === '--' && player.teeTime) {
